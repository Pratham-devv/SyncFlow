...
const DroppableColumn = ({ id, title, tasks, colorClass, borderClass, onEdit, onDelete, onStatusChange, onAddMember }) => {
  ...
  return (
    <div className="flex flex-col w-[85vw] sm:w-[320px] xl:flex-1 shrink-0 snap-center bg-slate-100/50 rounded-2xl border border-slate-200 p-4 h-[calc(100vh-200px)]">
      <div className={`flex items-center gap-2 mb-4 pb-3 border-b ${borderClass} shrink-0`}>
        <div className={`w-3 h-3 rounded-full ${colorClass}`} />
        <h3 className="font-bold text-slate-800" style={{ fontFamily: 'var(--font-heading)' }}>{title}</h3>
        <span className="ml-auto bg-white border border-slate-200 text-slate-500 text-xs font-bold px-2 py-0.5 rounded-md">
          {tasks.length}
        </span>
      </div>
      <SortableContext id={id} items={tasks.map(t => t._id)} strategy={verticalListSortingStrategy}>
        <div ref={setNodeRef} className="flex-1 overflow-y-auto space-y-3 pr-1 pb-4 min-h-[100px]">
          ...
          {tasks.length === 0 && (
            <div className="text-center py-8 text-slate-400 text-sm border-2 border-dashed border-slate-200 rounded-xl">Drop here</div>
          )}
        </div>
      </SortableContext>
    </div>
